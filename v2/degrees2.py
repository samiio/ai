...
def main():
    ...
    if path is None:
        ...
        for i in range(degrees):
            person1 = getNameFromId(queryPeople, str(path[i][1]))
            person2 = getNameFromId(queryPeople, str(path[i + 1][1]))
            movie = getNameFromId(queryMovies, str(path[i + 1][0]))
...
def loadData():
    ...
    for pid in pids:
        people[str(pid)] = { "movies": set() }

    # Load movies
    mids = getIds("SELECT id FROM movies")
    for mid in mids:
        movies[str(mid)] = { "stars": set() }
...
def personIdForName(name):
    ...
    for person in births:
        personDict[str(person[0])] = person[1] + ", " + str(person[2])
